...
React 19 + TypeScript 单页客户端，Node.js (Express + ws) 游戏服务器，WebSocket 实时双向通信。服务端权威架构：所有游戏逻辑（匹配、抛硬币、出牌判定、结算区重算）在服务器执行，客户端只负责渲染和发送意图。内置 BotAI 模拟真实玩家行为。一套 `shared/protocol.ts` 定义前后端类型契约和消息协议。支持 `systemd`/`pm2` 部署，前端可脱离后端独立静态托管。
...
| **抛硬币** | PvP：猜对者优先选卡组；机器人：玩家先选牌组再抛硬币决定先手 |
...
│   │   ├── Game.tsx         # 对战页（抛硬币/选卡组/出牌配对/结算/显示隐藏解释/结算区关联）
│   │   ├── Editor.tsx       # 卡组编辑器
│   │   └── Home.tsx         # 首页
│   ├── components/
│   │   └── Card.tsx         # 卡牌组件
