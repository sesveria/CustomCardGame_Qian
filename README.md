...
> 🎯 **这是什么？** 一款将知识点变成卡牌的双人对战游戏。手牌和公共牌池中藏着概念之间的关联——找到它们、配对得分，把卡组里的知识关联转化为你的分数。内置物理、英语等学科卡组，也支持自建卡组。可以和好友联机切磋，也可以随时和 AI 机器人来一局。

<details>
<summary><b>🔧 技术视角</b></summary>

React 19 + TypeScript 单页客户端，Node.js (Express + ws) 游戏服务器，WebSocket 实时双向通信。服务端权威架构：所有游戏逻辑（匹配、先手分配、出牌判定、结算区重算）在服务器执行，客户端只负责渲染和发送意图。内置 BotAI 模拟真实玩家行为。一套 `shared/protocol.ts` 定义前后端类型契约和消息协议。支持 `systemd`/`pm2` 部署，前端可脱离后端独立静态托管。
...
| **随机先手** | 系统随机指定先手玩家和选牌方，无需抛硬币 |
| **配对得分 + 结算区** | 配对得分累加，结算区内所有关联实时重算总分 |
| **显示/隐藏解释** | 一键切换所有卡牌描述文本的显示与隐藏 |
| **结算区关联** | 游戏结束弹窗展示己方结算区全部卡牌关联及分值 |
| **换牌策略** | 可将手牌弃入公共牌池换取新牌 |
...
│   │   ├── Game.tsx         # 对战页（选卡组/出牌配对/结算/显示隐藏解释/结算区关联）
│   │   ├── Editor.tsx       # 卡组编辑器
│   │   └── Home.tsx         # 首页
│   ├── components/
│   │   └── Card.tsx         # 卡牌组件
